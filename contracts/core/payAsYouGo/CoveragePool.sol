...
contract CoveragePool is ICoveragePool, BaseUpgradeablePausable {
    ...
    mapping(address => mapping(uint256 => mapping(uint256 => mapping(uint256 => uint256)))) private underwritersBalance;
    ...
    function withdraw(
        ...
        uint256 userBalance = calculateUnderwriterBalance(categoryID, subCategoryID);
    ...
    function calculateUnderwriterBalance(
        ...
        for(uint256 i = startVersionID; i <= currVersion;) {
            ...
            liquidatedAmount += (userBalance - (userBalance * liquidation) / 100);
